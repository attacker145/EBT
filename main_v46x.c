...
static long WlanConnect()
{
    ...
    long lRetVal = 0;
    cred_flg = 1;//Get SSID name and password from memory
//#ifdef cred
    if(cred_flg == 1){
...
*/
static int readPageResponse(HTTPCli_Handle httpClient)//Read data.txt page. Active function
{
    ...
    if(lRetVal > 0)
    {
        ...
        case 200:
        {
            ...
            output = strstr (dataBuffer,ID);//Find current ID value (string) in the received buffer
            if (output){
                //UART_PRINT("\nID is correct  ");
                ID_flg = 1;//ID is correct
            }else{
                UART_PRINT("\nID is not correct  ");
                ID_flg = 0;//ID is not correct
            }
            *output = NULL;
...
#define snr
        ...
        st_dev2 = sonar_sensrB(&mean_SNSR2, stl_data2, T_snr);//Motion detection and distance measurement Sensor B
        st_dev2 = noise_suppr (st_dev2, &mean_SNSR2, stl_data4, stl_data2);//Noise suppression.
        prnt_sonar (st_dev1, st_dev2, mean_SNSR1, mean_SNSR2);//Print score and distance in counts 617 counts per inch
#endif
        ADCsum = adc_read (&run);
       ...
       if(mean_SNSR1 > 10000 || mean_SNSR2 > 10000){
           if (IR_AVE > 550){//10000/617 = 16.2 inch.
               ebt = 1;
       ...
       UART_PRINT(" MAC= %llX ",(*(uint64_t *) macAddressVal) & 0x0000FFFFFFFFFFFF);
       //UART_PRINT(" IR1 = %f IR2 = %f IR3 = %f IR3 = %f \n",(float)IR1, (float)IR2, (float)IR3, (float)IR4);
       GPIO_IF_GetPortNPin(SH_GPIO_9,&uiGPIOPort,&pucGPIOPin);
       ucPinValue = GPIO_IF_Get(SH_GPIO_9,uiGPIOPort,pucGPIOPin);//Read RED LED status
       if (ucPinValue == 1){
           cx = snprintf(buf, 149, "IR1=%.0f & IR2=%.0f & IR3=%.0f & IR4=%.0f & RoomT=%.2f & TName=controllall & BLE=ON & SN=%.0f & ms=%llX",
                         (float)IR1,
                         (float)IR2,
                         (float)IR3,
                         (float)IR4,
                         sensorTemp,
                         (float)mean_SNSR1,
                         (*(uint64_t *) macAddressVal) & 0x0000FFFFFFFFFFFF);
       }else{
           cx = snprintf(buf, 149, "IR1=%.0f & IR2=%.0f & IR3=%.0f & IR4=%.0f & RoomT=%.2f & TName=controllall & BLE=OFF & SN=%.0f & ms=%llX",
                         (float)IR1,
                         (float)IR2,
                         (float)IR3,
                         (float)IR4,
...
#ifdef snr
       if (st_dev1 > STDEVTH){//Capture motion
           strcpy ( &buf[cx], " & temp=X ");//Buffer is cleared when snprintf(buf, 149, ...) above is executed
       }else{
           strcpy ( &buf[cx], " & temp=  ");
       }
 ...
 */
       if ((((st_dev1>STDEVTH || st_dev2>STDEVTH) || ebt==1) && t_cntr>30) || t_cntr2 == 90){//Post data
            if(cnn_fail == 0){
                t_cntr = 0;
                ebt = 0;
                if (t_cntr2 > 89)
        ...
        if(t_cntr1 > 5){//Get command
            //UART_PRINT("buf:%s \n\r", buf);//print buffer to be sent
